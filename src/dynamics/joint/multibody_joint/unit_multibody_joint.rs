...
pub fn unit_joint_limit_constraint(
    ...
    // Clamp the positional bias like the contact solver does: an uncapped 1/dt
    // limit bias turns a mm-scale violation at coarse timesteps into a m/s
    // velocity kick (measured kJ-scale energy spikes on a 33kg humanoid).
    let rhs_bias = (((curr_pos - limits[1]).max(0.0) - (limits[0] - curr_pos).max(0.0))
        * erp_inv_dt)
        .clamp(
...
/// Initializes and generate the velocity constraints applicable to the multibody links attached
/// to this multibody_joint.
pub fn unit_joint_motor_constraint_canary() {
    use std::sync::Once;
    static ONCE: Once = Once::new();
    ONCE.call_once(|| std::eprintln!("[rapier-patched] multibody unit motor path ACTIVE"));
}
...
pub fn unit_joint_motor_constraint(
    ...
    unit_joint_motor_constraint_canary();
